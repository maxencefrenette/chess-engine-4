...
The exact-batch confirmation ran at the smallest horizon accepted by the fitted
minimum-step rule: 20,238 steps, batch 20,480, and training ratio
`0.0490575349387x`. It used commit `8cc8fd49`, seed 1, the canonical training
data volume, and this command:

```sh
uv run train-modal --d-model 1280 --training-ratio 0.049057534938710845 \
  --wandb-name dense-minsteps-lr16d-d1280-m130-exact
```

The run reached final EMA loss `2.8946`, policy top-1 EMA `0.4943`, and
`EG_flops 1.340x`, but recorded two loss spikes. That would normally disqualify
an LR-tuning arm. After reviewing the result, the user explicitly accepted the
spikes and approved promotion as the exact `16d` d1280 baseline. The run is now
a canonical dense scaling observation and confirms the existing `1.30x`
multiplier. It cost `$0.837` from its 454.622-second W&B runtime. [W&B](https://wandb.ai/maxence-frenette/uncategorized/runs/06ai7vtr)
...
| d1280 | stale `19.2d`: `1.15: 2.8768` (1 spike), `1.30: 2.8680`; exact `16d`: `1.30: 2.8946` (2 accepted spikes) | `1.30x` |
...
requested two-run d1280 extension cost `$2.218`, and the exact-batch confirmation
cost `$0.837`, for `$10.460` cumulatively. The ladder commands are generated by
`launch_lr.py` and `launch_steps.py`; the exact-batch confirmation command is
recorded above. The confirmation landed in the W&B `uncategorized` project
because this worktree had no `.env`, but its full run record is linked above.
...
smooth `32d`-derived boundary above. The explicitly reviewed exact-batch run
confirms the d1280 `1.30x` entry in the `16d` learning-rate dictionary and is
promoted into the canonical dense scaling observations.
