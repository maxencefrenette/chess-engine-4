...
def test_policy_config_builds_connected_opening_waves() -> None:
    ...
    assert all(payload["opening_seed"] == 1 for payload in payloads)
    assert all(payload["opening_offset"] == 0 for payload in payloads)
...
""",
    ...
    assert match.opening_ids == (
        "/uho.pgn|seed=7|index=100",
        "/uho.pgn|seed=7|index=101",
    )
